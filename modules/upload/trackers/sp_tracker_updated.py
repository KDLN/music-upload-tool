...
class SPTracker(GenericTracker):
    ...
    def _build_form_data(self, metadata: Dict[str, Any], description: str) -> Dict[str, Any]:
        ...
        # Get category ID based on the release type (album, single, etc.) or content type
    ...
    def upload(self,
        ...
        # Add cover image if available - use the correct field name 'torrent-cover' for SP tracker
        if cover_path and os.path.exists(cover_path):
            ...
            logger.info(f"Added cover art to tracker upload: {cover_path}")
        ...
        try:
            ...
            if not response.ok and status_code == 422:
                ...
                # Provide specific guidance if it's likely a music category issue
                is_music_content = metadata.get('format', '').upper() in ['FLAC', 'MP3', 'AAC', 'OGG', 'WAV', 'ALAC'] or \
                                  'album' in metadata or 'release_type' in metadata
