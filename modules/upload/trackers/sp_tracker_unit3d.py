"""
SP tracker module with optimized cover handling for Seedpool based on UNIT3D.
"""
...
# Import Pillow for image resizing (if available)
try:
...
class SPTracker(GenericTracker):
    ...
    def _prepare_cover_image(self, metadata: Dict[str, Any]) -> Optional[str]:
        ...
        Scales the image to a reasonable size and converts to JPEG.
        ...
        # Prepare output path
        output_path = os.path.join(temp_dir, "album_cover.jpg")
        
        # Try to resize the image if PIL is available
        if HAS_PIL:
            try:
                logger.info(f"Preparing cover image from {cover_path}")
                ...
                logger.info(f"Successfully prepared cover: {output_path}")
                return output_path
    ...
    def upload(self,
        ...
        # Build file payload - torrent file
        files = {
            'torrent': (
        ...
        # Add cover file if available - using field name from UNIT3D codebase
        if cover_path and os.path.exists(cover_path):
            try:
                # Use the correct field name 'torrent_cover' from UNIT3D validation rules
                files['torrent_cover'] = (
                    'album_cover.jpg',
                    open(cover_path, 'rb'),
                    'image/jpeg'
                )
                logger.info(f"Added cover image as 'torrent_cover': {cover_path}")
            except Exception as e:
        ...
        try:
            ...
            logger.info(f"Response content: {response.text[:200]}...")  # Log some of the response
            ...
            if not response.ok and status_code == 422:
                ...
                logger.error(f"Validation error detected! Response: {response.text[:1000]}")
